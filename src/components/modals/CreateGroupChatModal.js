...
function CreateGroupChatModal({ setisCreateGCToggle }) {
  ...
  const [markedMembers, setmarkedMembers] = useState([])

  const valueToArrayChecker = (userID) => {
    var userIDExistInArray = markedMembers.filter((flt, i) => flt.userID == userID)

    return userIDExistInArray.length > 0? true : false
  }

  const removeFromList = (userID) => {
    var userIDnotSimilar = markedMembers.filter((flt, i) => flt.userID != userID)
