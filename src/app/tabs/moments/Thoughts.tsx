...
export function ThoughtDetail({
  ...
  const [mine, setMine] = useState<string | null>(
    (thought as any).my_reaction ?? null,
  );
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <span
          style={{
            display: "inline-flex",
            borderRadius: "50%",
            boxShadow: "0 0 0 2px var(--surface), 0 0 0 4px var(--brand)",
            flex: "none",
          }}
        >
          <Avatar
        ...
        >
          ...
          >
            {timeAgoLabel(thought.date_posted)} · <Icon n="timer" s={12} />
            {timeLeftLabel(thought.expires_at)}
          </span>
        </div>
        <button
          onClick={onClose}
            ...
            style={{
              ...
              background:
                mine === emoji.emoji_id
                  ? "var(--brand-soft)"
                  : "var(--surface-2)",
              border: `1px solid ${mine === emoji.emoji_id ? "var(--brand)" : "var(--border)"}`,
              fontSize: 19,
              cursor: "pointer",
            }}
          >
      ...
      >
        <Icon n="lock" s={14} />
        Replies go to your chat with {first}.
      </span>
    </div>
  );
}
 ...
 * and offers Delete.
 */
export function ThoughtComposerModal({
  existing,
  onClose,
}: {
  existing: IThought | null;
  onClose: () => void;
}) {
  const alert = useAlert();
  ...
  const [mood, setMood] = useState<ThoughtMood | null>(
    existing?.content.mood ?? null,
  );
  const [audience, setAudience] = useState<EphemeralAudience>(
    (existing?.privacy_status as EphemeralAudience) ??
  ...
  useEffect(() => {
    if (existing)
      GetOwnThoughtRequest(existing.post_id)
        .then((t) => setViews(t.views ?? 0))
        .catch(() => {});
  }, [existing?.post_id]);
  ...
  const save = async () => {
    ...
    try {
      if (existing) {
        await UpdateThoughtRequest(existing.post_id, {
          text: text.trim(),
          mood,
          privacy_status: audience,
        });
        done("Thought updated.");
      } else {
        await CreateThoughtRequest({
          text: text.trim(),
          mood,
          privacy: audience,
        });
    ...
    <Modal
      component={
        ...
        >
          ...
          >
            <ThoughtBubble
              text={text.trim() || "What's on your mind?"}
              mood={mood}
              style={{ maxWidth: 240, alignItems: "center" }}
            />
            <div style={{ width: 100 }}>
              <BubbleTail left={30} />
            </div>
            ...
            {existing && (
              ...
              >
                <Icon n="timer" s={14} />
                {timeLeftLabel(existing.expires_at)}
                {views !== null ? ` · seen by ${views}` : ""}
              </span>
            )}
          </div>
          ...
          <div
            style={{
              ...
              padding: "0 12px",
              background: "var(--input)",
              border: `1px solid ${length > THOUGHT_MAX_LENGTH ? "var(--pink)" : "var(--border)"}`,
              borderRadius: "var(--r-sm)",
              marginBottom: 6,
            }}
          >
            ...
            <span
              style={{
                fontSize: "var(--fs-meta)",
                color:
                  length > THOUGHT_MAX_LENGTH ? "var(--pink)" : "var(--text-3)",
              }}
            >
          ...
          <div style={{ display: "flex", gap: 4, marginBottom: 16 }}>
            {THOUGHT_EMOJIS.map((emoji) => (
              <button
                ...
                onClick={() =>
                  setText((t) =>
                    charCount(t) < THOUGHT_MAX_LENGTH ? `${t}${emoji}` : t,
                  )
                }
                style={{
                  flex: 1,
          ...
          <SegTabs
            tabs={AUDIENCES.map((a) => ({
              key: a.key,
              label: a.label,
              icon: a.icon,
            }))}
            value={audience}
            onChange={(k) => setAudience(k as EphemeralAudience)}
          ...
          >
            ...
            >
              Disappears after 24 hours. Not shown in the feed.
            </span>
            <Btn
              onClick={save}
              disabled={busy || length === 0 || length > THOUGHT_MAX_LENGTH}
            >
...
export function ThoughtsRail() {
  ...
  const [open, setOpen] = useState<{ thought: IThought; left: number } | null>(
    null,
  );
  ...
  const load = () =>
    GetThoughtsRailRequest()
      .then(setRail)
      .catch(() => setRail({ mine: null, results: [] }));
  ...
  const people = useMemo(() => {
    ...
    const list = (rail?.suggestions ?? []).filter(
      (entity) => !withThought.has(entity.id),
    );
    const online = list.filter((entity) =>
      isUserOnline(activeUsers, entity.id),
    );
    const offline = list.filter(
      (entity) => !isUserOnline(activeUsers, entity.id),
    );
    ...
    <div style={{ position: "relative", marginBottom: 6 }}>
      ...
      >
        {item(
          ...
          <ThoughtBubble
            size="sm"
            text={rail.mine?.content.text ?? "Share a thought"}
            style={{
              maxWidth: 88,
              minWidth: 44,
        ...
        {rail.results.map((thought) =>
          item(
            ...
            <ThoughtBubble
              ...
              highlighted={open?.thought.post_id === thought.post_id}
              style={{ maxWidth: 88, minWidth: 44 }}
            />,
            <Avatar
              id={thought.author?.id}
              entityId={thought.author?.id}
              name={entityName(thought.author)}
              src={entityAvatar(thought.author)}
              size={52}
            />,
            ...
            (e) => {
              ...
              setOpen(
                open?.thought.post_id === thought.post_id
                  ? null
                  : { thought, left: Math.max(0, at.left - host.left - 10) },
              );
            },
          ),
        )}
