...
function CallWindow({ data, lineNum }: any) {
  ...
  const mediamyvideoholder = useSelector(
    (state: any) => state.mediamyvideoholder,
  );
  ...
  const [icecandidatestrigger, seticecandidatestrigger] = useState<any | null>(
    null,
  );
  const [answerresponsetrigger, setanswerresponsetrigger] = useState<
    any | null
  >(null);

  const [localGlobalPeer, setlocalGlobalPeer] = useState<PeerService | null>(
    null,
  );
    ...
    if (data.conversationType == "single") {
      if (data.caller.userID == authentication.user.userID) {
        EndCallRequest({
          ...
          conversationID: data.conversationID,
          recepients: data.recepients.filter(
  ...
  return (
    <motion.div
      ...
      dragConstraints={{
        top: 0,
        bottom: windowHeight - 185,
        left: 0,
        right: windowWidth - 315,
      }}
    ...
    >
      ...
      {isAnswered /** isAnswered by default and ! only for testing  */ ? (
        ...
        <div className="div_callwindow_content_not_answered">
          <span className="span_callwindow_content_not_answered_label">
            ...waiting for {data.callDisplayName}
          </span>
        </div>
      ) : (
        <div className="div_callwindow_content_not_answered">
